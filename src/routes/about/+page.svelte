...
<div class="min-h-screen max-w-screen px-0 m-0 overflow-x-clip overscroll-x-none" style="font-family: Inter">
    <header class="mx-auto flex items-center text-white lg:max-w-screen-xl md:w-3/5 w-4/5 overscroll-x-none">
        <div class="text-left mt-20 w-full">
            <h1 class="text-xl my-0 text-white brightness-25 hover:brightness-100 duration-1000 transition-ease-out">{date}</h1>
            <div class="rise-wrapper whitespace-pre w-full font-800 -spacing-1 text-white">
                {#each title.split(" ") as word, index}
                    <p class="rise-anim inline-flex text-5xl lg:text-7xl my-0 font-800 -spacing-3 mt-6" style={`--delay: ${index * 200 + 100}ms`}
                        >{`${word}`}{index !== title.length-1 ? " " : ""}</p>
				{/each}
            </div>
            <hr class="my-10 border-[#332255]"/>
        </div>
    </header>
    <div class="text-lg text-white lg:max-w-screen-xl md:w-3/5 w-4/5 mx-auto overflow-x-clip overscroll-x-none pb-10">
